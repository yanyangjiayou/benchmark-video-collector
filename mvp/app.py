...
def worker(payload: StartPayload) -> None:
    try:
        ...
        if payload.request.platform == "dy" and "抖音登录已失效" in str(exc):
            login_state.update(status="not_logged_in", platform="dy", message="抖音登录已失效，请重新确认登录")
...
def login_worker(platform: str) -> None:
    ...
    try:
        ...
        login_state.update(status="not_logged_in", message=message)
    except subprocess.TimeoutExpired:
        login_state.update(status="not_logged_in", message="登录检查超时，请重新确认登录并在平台窗口完成扫码或验证。")
    except Exception as exc:
        login_state.update(status="not_logged_in", message=f"无法启动登录检查：{exc}")
...
@media(max-width:600px){body{font-size:15px}body:after{width:170px;min-width:0;height:120px}.app-header{align-items:flex-start}.local-badge{display:none}.app-header h1{font-size:23px}.brand-mark{width:42px;height:42px}.card{padding:19px;border-radius:16px}.history{padding:0}.history .card-head{padding:18px}.archive-count{min-width:64px;font-size:20px}.grid{grid-template-columns:1fr}.actions{grid-template-columns:1fr}.actions .ghost{grid-column:auto;grid-row:auto}.metrics{grid-template-columns:repeat(2,1fr)}th,td{padding:10px}.file-name{max-width:180px}}
...
<header class="app-header"><div class="title-wrap"><div class="brand-mark" aria-hidden="true">采</div><div><h1>对标视频采集助手</h1><p>公开内容采集 · 本地转写 · Excel 归档</p></div></div><div class="local-badge">仅在本机运行</div></header>
...
<label>博主账号备注（可选）<input id="account" placeholder="用于历史记录和 Excel；不填则读取昵称"></label>
...
<div class="actions"><button class="ghost" onclick="preview()">检查规则</button><button id="loginButton" class="secondary" onclick="checkLogin()">1. 确认登录</button><button id="startButton" onclick="startJob()" disabled>2. 开始采集</button></div>
<div id="loginStatus">登录状态：尚未检查</div><div id="rulePreview"></div></div>
<aside class="card status-card"><div class="card-head"><div><span class="eyebrow">Live status</span><h2>任务状态</h2></div><span id="statusPill" class="status-pill">等待操作</span></div><div id="status"><div class="status-lead"><b>采集状态：尚未开始</b><div>完成登录确认后，即可开始采集。</div></div></div></aside></section>
...
function platformChanged(){creatorUrls[selectedPlatform]=el('url').value;selectedPlatform=el('platform').value;el('url').value=creatorUrls[selectedPlatform]||'';el('url').placeholder=selectedPlatform==='dy'?'粘贴抖音博主主页链接或主页分享文字':'粘贴小红书博主主页链接';el('startButton').disabled=true;el('loginStatus').textContent='登录状态：切换平台后请重新确认'}
...
async function pollLogin(){try{let s=await api('/api/login-status'),matched=s.status==='logged_in'&&s.platform===el('platform').value;el('loginStatus').textContent='登录状态：'+(s.platform&&s.platform!==el('platform').value?'当前平台尚未确认登录':s.message);el('startButton').disabled=!matched;el('loginButton').disabled=s.status==='checking';el('loginButton').textContent=s.status==='checking'?'正在检查…':matched?'重新确认登录':'1. 确认登录';if(s.status==='checking')setTimeout(pollLogin,1200)}catch(error){el('loginStatus').textContent='登录状态暂时无法读取，请重新确认登录';el('startButton').disabled=true;el('loginButton').disabled=false;el('loginButton').textContent='1. 确认登录'}}
...
function render(s){let p=s.progress||{}, names={idle:'尚未开始',running:'正在采集',done:'采集完成',error:'采集失败'},pillNames={idle:'等待操作',running:'任务进行中',done:'已完成',error:'需处理'};
let message=s.status==='idle'?'请确认登录，然后点击“开始采集”。':s.status==='running'?'程序正在执行，请保持页面打开。':s.status==='done'?(p.exported?`采集完成，共导出 ${p.exported} 条新结果，新增 ${p.history_marked||0} 条持久化标记。${p.media_missing?`其中 ${p.media_missing} 条未取得视频文件，下次仍可重试。`:''}`:(p.history_skipped?`没有发现新内容，已跳过 ${p.history_skipped} 条历史采集内容。`:'采集完成，但没有符合条件的新数据。')):`采集失败：${s.error||'请展开运行记录查看原因'}`;
...
let timing=s.status==='running'?`<div class="timing">当前阶段：${esc(p.stage||'正在处理')}。已用 ${duration(elapsed)}，预计还需约 ${duration(remaining)}。时间为估算值，取决于视频长度和网络速度。</div>`:'';
el('statusPill').textContent=pillNames[s.status]||s.status;el('status').innerHTML=`<div class="status-lead"><b>采集状态：${names[s.status]||s.status}</b><div>${message}</div></div>${timing}<div class="metrics"><div class="metric"><b>${p.found??0}</b>发现内容</div><div class="metric"><b>${p.videos??0}</b>视频</div><div class="metric"><b>${p.eligible??0}</b>本次新内容</div><div class="metric"><b>${p.history_skipped??0}</b>历史已跳过</div><div class="metric"><b>${p.content_duplicates??0}</b>内容重复</div><div class="metric"><b>${p.transcribed??0}</b>完成转写</div><div class="metric"><b>${p.media_missing??0}</b>未完成</div></div>`;
...
async function poll(){let x=await fetch('/api/status'),s=await x.json();render(s);if(s.status==='running')setTimeout(poll,1500);else if(s.status==='done')loadExports()}
