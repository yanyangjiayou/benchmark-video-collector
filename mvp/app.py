...
def worker(payload: StartPayload) -> None:
    ...
    try:
        ...
        elif "登录已失效" in message:
            platform_name = "小红书" if payload.request.platform == "xhs" else "抖音"
            platform_state.update(status="not_logged_in", message=f"{platform_name}登录已失效，请重新确认登录")
    finally:
        with task_lock:
            active_process = None
...
@media(max-width:600px){body{font-size:15px}body:after{width:170px;min-width:0;height:120px}.app-header{align-items:flex-start}.local-badge{display:none}.app-header h1{font-size:23px}.brand-mark{width:42px;height:42px}.card{padding:19px;border-radius:16px}.history{padding:0}.history .card-head{padding:18px}.archive-count{min-width:64px;font-size:20px}.grid{grid-template-columns:1fr}.actions,.safety-strip{grid-template-columns:1fr}.actions .ghost,.recovery{grid-column:auto;grid-row:auto}.metrics{grid-template-columns:repeat(2,1fr)}th,td{padding:10px}.file-name{max-width:180px}}
...
<label id="fanbox">最低粉丝数（可选）<input id="fans" type="number" min="0" placeholder="不填则不限制"></label>
...
function toggle(){let k=el('trigger').value==='keyword';el('urlbox').style.display=k?'none':'block';el('keybox').style.display=k?'block':'none';el('fanbox').style.display=el('platform').value==='xhs'?'block':'none'}
...
if(el('platform').value==='xhs'&&el('fans').value)r.min_followers=Number(el('fans').value);return {request:r,account_label:el('account').value,transcription_model:el('model').value,glossary:el('glossary').value}}
